...
elgg.db_explorer.init = function () {
	...
	$(document).on('change', '.js-dbexplorer-toggle', function () {
		var checked = $(this).prop('checked');
		$('.jqgrid-wrapper').find('.js-dbexplorer-checkbox').prop('checked', false);
		$('.jqgrid-wrapper').find('.js-dbexplorer-checkbox:visible').prop('checked', checked);
	});

	$(document).on('submit', '.js-dbexplorer-batch-form', function (e) {
		e.preventDefault();

		var $form = $(this);
		var data = $('.jqgrid-wrapper').find('.js-dbexplorer-checkbox:checked:visible').serialize();

		elgg.action($form.attr('action') + '?' + data, {
			data: $form.serialize(),
			beforeSend: function () {
	...
	$(document).on('click', '.dbexplorer-popup', function (e) {
		e.preventDefault();

		var $popup = $('<div>').html($('<div>').addClass('elgg-ajax-loader'));

		$popup.dialog({
			title: i18n.echo('db_explorer:loading'),
			width: 1300,
			height: 600,
			close: function () {
	...
	// Highlight
	$(document).on('mouseenter', 'td[role="gridcell"]:has([data-guid])', function (e) {
		var guid = $(this).find('[data-guid]').eq(0).data('guid');
		$('[data-guid="' + guid + '"]').closest('td[role="gridcell"]').addClass('highlighted');
	}).on('mouseleave', 'td[role="gridcell"]', function (e) {
...
elgg.db_explorer.jqGrid = function ($grid) {
	...
	switch (type) {
		...
		case 'site' :
			...
			colModel.push(
					{name: 'se.name', width: 100, searchoptions: {sopt: ['eq', 'bw', 'bn', 'nc', 'ew', 'en']}},
	...
	{name: 'e.enabled', width: 90, searchoptions: {sopt: ['eq', 'ne']}}
	);

	$grid.jqGrid({
		url: security.addToken(elgg.get_site_url() + 'action/db_explorer/entities?' + $.param($grid.data())),
		datatype: 'json',
		mtype: 'GET',
		colNames: colNames,
		colModel: colModel,
...
elgg.db_explorer.subGridRowExpanded = function (subgrid_id, row_id) {
	...
	$.each(subgrid_tables, function (key, subgrid_table_name) {
		...
		$('#' + subgrid_accordion_id).append('<h3>' + i18n.echo('db_explorer:tables:' + subgrid_table_name) + '</h3>');
		...
		switch (subgrid_table_name) {
			case 'users_entity' :
				jQuery('#' + subgrid_table_id).jqGrid({
					url: security.addToken(elgg.get_site_url() + 'action/db_explorer/users_entity?guid=' + row_id),
					datatype: 'json',
					colNames: ['checkbox', 'guid', 'name', 'username', 'email', 'language', 'banned', 'admin', 'last_action', 'prev_last_action', 'last_login', 'prev_last_login'],
					colModel: [
						{
							name: 'checkbbox',
							width: 30,
			...
			case 'objects_entity' :
				jQuery('#' + subgrid_table_id).jqGrid({
					url: security.addToken(elgg.get_site_url() + 'action/db_explorer/objects_entity?guid=' + row_id),
					datatype: 'json',
					colNames: ['checkbox', 'guid', 'title', 'description'],
					colModel: [
						{
							name: 'checkbbox',
							width: 30,
			...
			case 'groups_entity' :
				jQuery('#' + subgrid_table_id).jqGrid({
					url: security.addToken(elgg.get_site_url() + 'action/db_explorer/groups_entity?guid=' + row_id),
					datatype: 'json',
					colNames: ['checkbox', 'guid', 'name', 'description'],
					colModel: [
						{
							name: 'checkbbox',
							width: 30,
			...
			case 'sites_entity' :
				jQuery('#' + subgrid_table_id).jqGrid({
					url: security.addToken(elgg.get_site_url() + 'action/db_explorer/sites_entity?guid=' + row_id),
					datatype: 'json',
			...
			case 'contained_entities' :
				jQuery('#' + subgrid_table_id).jqGrid({
					url: security.addToken(elgg.get_site_url() + 'action/db_explorer/' + subgrid_table_name + '?guid=' + row_id),
					datatype: 'json',
					colNames: [
						'checkbox',
						'guid',
			...
			case 'river_items' :
				jQuery('#' + subgrid_table_id).jqGrid({
					url: security.addToken(elgg.get_site_url() + 'action/db_explorer/river_items?guid=' + row_id),
					datatype: 'json',
			...
			case 'access_collections_ownership' :
				jQuery('#' + subgrid_table_id).jqGrid({
					url: security.addToken(elgg.get_site_url() + 'action/db_explorer/access_collections_ownership?guid=' + row_id),
					datatype: 'json',
			...
			case 'access_collections_membership' :
				jQuery('#' + subgrid_table_id).jqGrid({
					url: security.addToken(elgg.get_site_url() + 'action/db_explorer/access_collections_membership?guid=' + row_id),
					datatype: 'json',
					colNames: ['id', 'name', 'owner_guid', 'site_guid'],
					colModel: [
						{name: 'acl.id'},
			...
			case 'private_settings' :
				jQuery('#' + subgrid_table_id).jqGrid({
					url: security.addToken(elgg.get_site_url() + 'action/db_explorer/private_settings?guid=' + row_id),
					datatype: 'json',
			...
			case 'entity_relationships' :
				jQuery('#' + subgrid_table_id).jqGrid({
					url: security.addToken(elgg.get_site_url() + 'action/db_explorer/entity_relationships?guid=' + row_id),
					datatype: 'json',
