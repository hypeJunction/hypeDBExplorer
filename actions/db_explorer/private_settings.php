...
$sidx = get_input('sidx', 'ps.id');
$sord = get_input('sord', 'asc');

$guid = get_input('guid', 0);

$dbprefix = elgg_get_config('dbprefix');
$row_count = array_map(static fn($r) => (object) $r, elgg()->db->getConnection('read')->executeQuery("SELECT COUNT(*) AS count FROM {$dbprefix}private_settings WHERE entity_guid = $guid")->fetchAllAssociative());
...
$row_data = array_map(static fn($r) => (object) $r, elgg()->db->getConnection('read')->executeQuery("SELECT * FROM {$dbprefix}private_settings ps
						WHERE ps.entity_guid = $guid
						ORDER BY $sidx $sord
						LIMIT $limit 
						OFFSET $offset"
)->fetchAllAssociative());
