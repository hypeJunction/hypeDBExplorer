...
$sidx = get_input('sidx', 'r.id');
$sord = get_input('sord', 'asc');

$guid = get_input('guid', 0);

$dbprefix = elgg_get_config('dbprefix');
$row_count = array_map(static fn($r) => (object) $r, elgg()->db->getConnection('read')->executeQuery("SELECT COUNT(*) AS count FROM {$dbprefix}river WHERE subject_guid = $guid OR object_guid = $guid")->fetchAllAssociative());
...
$row_data = array_map(static fn($r) => (object) $r, elgg()->db->getConnection('read')->executeQuery("SELECT * FROM {$dbprefix}river r
...
if (!empty($row_data)) {
	...
	$ordered_cols = [
		'id', 'type', 'subtype', 'action_type', 'subject_guid', 'object_guid', 'annotation_id', 'access_id', 'view', 'posted',
	];
