...
$sidx = get_input('sidx', 'acl.id');
$sord = get_input('sord', 'asc');

$guid = get_input('guid', 0);

$dbprefix = elgg_get_config('dbprefix');
$row_count = \hypeJunction\DBExplorer\query("SELECT COUNT(*) AS count FROM {$dbprefix}access_collections WHERE owner_guid = $guid");
...
$row_data = \hypeJunction\DBExplorer\query("SELECT * FROM {$dbprefix}access_collections acl
						WHERE acl.owner_guid = $guid
						ORDER BY $sidx $sord
						LIMIT $limit 
						OFFSET $offset"
);
